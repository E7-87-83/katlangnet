...
- Ellipsis `...` is the POSTFIX sequence supply operator: `expr...` supplies the result sequence of `expr` (followed by nothing), and it is not a normal argument separator. It NEVER consumes a right operand: any token after `...` — tight, spaced, or on the next physical line — starts a new expression that joins by the ordinary output-chain rules, so `A...B`, `A...empty`, `A... B`, `A...` newline `B`, and `A... ; B` all parse as `(A...) ; B`. Postfix `expr...` does not continue onto the next physical line. Because `...` never owns a right operand, the source token after the dots is an ordinary output contribution, never the supply's right operand; internally `expr...` is a unary supply node over its single operand, with no right operand. To supply `expr`'s items followed by an extra value as separate call arguments, use a comma: `f(A..., B)` is a two-argument-style spread, while `f(A...B)` is `f((A...) ; B)` — one joined/grouped argument.
