...
- A user-defined top-level variadic parameter consumes an item stream. `Name(values...) = body` binds the supplied top-level items as one grouped sequence value. When the whole stream is exactly one grouped sequence value it is opened by singleton-boundary normalization (this repeats through singleton boundaries); explicit `expr...` opens its items into the same stream; multiple sibling sequence values are preserved unless explicitly opened. `Name()` binds the empty stream. A lone rest-only `values...` is the degenerate single-rest case of this model.
