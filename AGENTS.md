...
- Ellipsis `...` is the postfix spread operator. `expr...` spreads the evaluated sequence value of `expr` into the surrounding structural context. It never consumes a right operand: `A...B`, `A...empty`, `A... B`, and `A...` newline `B` all parse as expression lists beginning with `A...`. `A... ; B` is invalid semicolon syntax.
