...
- A property/call/builtin RESULT boundary is a value boundary: it always returns ONE value, with emitted count `Result.valueCount` (0 for the empty sequence value, otherwise 1). A multi-output body/collection is observed by the caller as one sequence value; only explicit caller-site postfix `...` re-opens it into the surrounding item stream (spread reads the value via `toItems`, not the emitted count). The shared helper is `reCountValueBoundary` (Lean) / `ReCountValueBoundary` (C#); it preserves the structural value EXACTLY (never `normalize`/`FromItems`, so `(())`/`((()))`/singletons are not collapsed) and is applied at user/conditional calls (`evalUserCallCounted`, `evalConditionalCallCounted`), structural dot zero-arg access (`evalDotCallCounted`), and collection-producing builtins (`order`, `orderDesc`, `distinct`, `take`, `skip`, `filter`, `map`, `range`, `atoms`). Lexical zero-arg property access and `if` already re-counted this way. NOT value boundaries (must keep their multi-item counts): root/body output accumulation (`evalAlgOutputCountedCore`), raw variadic/rest parameter storage in `countedParamEnv`/`VariadicStreamEnv` (internal forwarding such as `sum(a)`/`a...` relies on the raw count), `while`/`repeat` multi-slot loop state, and the strict single-value `map`/`reduce` callback contract. Scalar/reduction builtins already return one value and are unchanged.
...
- Ellipsis `...` is the postfix spread operator. `expr...` spreads the evaluated sequence value of `expr` into the surrounding structural context. It never consumes a right operand: `A...B`, `A...C`, `A... B`, and `A...` newline `B` all parse as expression lists beginning with `A...`. `A... ; B` is invalid semicolon syntax.
...
- A user-defined top-level variadic parameter consumes an item stream. `Name(values...) = body` binds the supplied top-level items as one grouped sequence value. When the whole stream is exactly one grouped sequence value it is opened once by singleton-boundary normalization (so `()` stays empty but `(())` opens to one item, the empty sequence value); explicit `expr...` opens its items into the same stream; multiple sibling sequence values are preserved unless explicitly opened. `Name()` binds the empty stream. A lone rest-only `values...` is the degenerate single-rest case of this model.
- A parameter list with two or more captures that contains one rest is a comma deconstruction — the general form of the same item-stream binding: fixed captures bind from the front and back, and the single movable rest captures the remaining middle items as one grouped sequence value (possibly zero). The same matcher governs assignment binding (`x, y..., z = RHS`, parser-elaborated into properties). The item-stream matcher is `bindParameterPatternList` (Lean) / `BindParameterPatternList` (C#) with singleton-boundary normalization (`normalizeSingletonBoundaryForItemStream` / `NormalizeSingletonBoundaryForItemStream`); routing is `Algorithm.usesItemStreamBinding` / `IsDeconstructionUserCallShape`. Callback-element binding is intentionally left on the strict path (callback deconstruction deferred).
- Ordinary lexical dot-call passes the receiver as one leading argument boundary. `A.B(C, D)` means `B(A, C, D)`, not a call where `A`'s top-level values are spread before `C` and `D`. If `B` has a leading `values...` parameter, that parameter binds the receiver slot as part of its item stream after ordinary receiver injection.
- Sequence builtins such as `filter`, `map`, `order`, `orderDesc`, `count`, `first`, `last`, `min`, `max`, `sum`, `avg`, and `reduce` expose a rest-shaped public signature (`sum(values...)`, `contains(values..., item)`) and therefore consume an **item stream**, through the same shared binder as user-defined variadics. `values...` has exactly one meaning across the language. The rest captures the collection (with the same singleton-boundary normalization — `count(Values)`, `count((1, 2, 3))`, and `count(1, 2, 3)` all count three items, and `count(Values...)` opens its items into the same stream), suffix parameters bind from the back (`contains(values..., item)` binds `contains(1, 2, 3, 2)` and `contains((1, 2, 3), 2)` alike), and multiple sibling grouped values are preserved unless explicitly opened with `...` (so `sum(A, B)` with grouped `A`/`B` is rejected by the numeric-item constraint rather than silently flattened, while `sum(A..., B...)` concatenates). A builtin that genuinely requires one collection value must declare a non-rest `values` parameter instead of `values...`. Builtin runtime binding routes through `bindCallableArguments` / `BindCallableArguments` (the shared item-stream kernel) after `normalizeSingletonBoundaryForItemStream`; the collection then comes from the rest capture opened by that same one-boundary normalization (`normalizeSingletonBoundaryForItemStream` / `NormalizeSingletonBoundaryForItemStream`) — exactly one grouped sequence boundary is opened where required, and it must NOT recursively renormalize through `Result.FromItems` / `Result.normalize`, which would erase structural sequence values such as `(())`. Collection-producing builtins combine their returned items with the structure-preserving `combineCollectionResult` / `CombineCollectionResult` path, and callback/builtin execution stays a separate runtime-owned phase after binding.
