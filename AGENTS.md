...
- Postfix `...` binds to its immediate operand before expression-list handling. `X(a b...)` and `X(a` newline `b...)` parse as `X(a, b...)`. To spread a value plus another argument as separate slots, use comma or adjacency: `f(A..., B)` and `f(A...B)` are both two-argument forms. To make one sequence-value argument containing a supply plus another value, write `f((A..., B))`.
