...
- Sequence builtins such as `filter`, `map`, `order`, `orderDesc`, `count`, `first`, `last`, `min`, `max`, `sum`, `avg`, and `reduce` keep a strict one-slot `values...` convention (unlike user-defined variadics, which use item-stream binding). Prefer one sequence-valued collection slot, such as `count(Values)`, `sum((10, 20, 30))`, or `take((1, 2, 3), 2)` for suffix builtins. Inline comma remains structural (`count(1, 2, 3)` is an arity error); `count(Values...)` is also an arity error when `Values...` opens more than the one collection slot.
