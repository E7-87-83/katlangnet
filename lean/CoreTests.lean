...
def emptyOutputRunsFlat : Bool :=
  match runFlat .emptyOutput with
  | Except.ok [] => true
  | _ => false

#eval emptyOutputRunsFlat  -- should be true

def emptyOutputDotCallCountIsZero : Bool :=
  match runFlat (.dotCall .emptyOutput "count" none) with
  | Except.ok [0] => true
  | _ => false

#eval emptyOutputDotCallCountIsZero  -- should be true

def emptyOutputPlainCountIsZero : Bool :=
  match runFlat (.call (resolve "count") (alg [] [] [] [.emptyOutput])) with
  | Except.ok [0] => true
  | _ => false

#eval emptyOutputPlainCountIsZero  -- should be true

def emptyOutputPropertyDotCallCountIsZero : Bool :=
  match runFlat (.block (algPrivate [] [] [
    ("A", alg [] [] [] [.emptyOutput])
  ] [
    .dotCall (resolve "A") "count" none
...
#eval emptyOutputPropertyDotCallCountIsZero  -- should be true

def emptyOutputNeutralInResultJoin : Bool :=
  match runFlat (.resultJoin (.num 1) (.resultJoin .emptyOutput (.num 2))) with
  | Except.ok [1, 2] => true
  | _ => false

#eval emptyOutputNeutralInResultJoin  -- should be true

def emptyOutputIndexIsBadIndex : Bool :=
  match runResult (.index .emptyOutput (.num 0)) with
  | Except.error err => innermostIsBadIndex err
  | _ => false

#eval emptyOutputIndexIsBadIndex  -- should be true

def runtimeEmptyCountStillZero : Bool :=
  match runFlat (.block (algPrivate [] [] [
    ("IsEven", alg ["x"] [] [] [.binary .eq (.binary .mod (.param "x") (.num 2)) (.num 0)])
  ] [
    .dotCall (.call (resolve "filter") (alg [] [] [] [
      .num 1,
      .num 3,
      .num 5,
      resolve "IsEven"
...
#eval runtimeEmptyCountStillZero  -- should be true

def emptyOutputEqualityIsTrue : Bool :=
  match runFlat (.binary .eq .emptyOutput .emptyOutput) with
  | Except.ok [1] => true
  | _ => false

#eval emptyOutputEqualityIsTrue  -- should be true

def runtimeEmptyEqualsEmptyOutput : Bool :=
  match runFlat (.block (algPrivate [] [] [
    ("IsEven", alg ["x"] [] [] [.binary .eq (.binary .mod (.param "x") (.num 2)) (.num 0)])
  ] [
    .binary .eq
...
#eval runtimeEmptyEqualsEmptyOutput  -- should be true

def emptyOutputInequalityIsFalse : Bool :=
  match runFlat (.binary .ne .emptyOutput .emptyOutput) with
  | Except.ok [0] => true
  | _ => false

#eval emptyOutputInequalityIsFalse  -- should be true
