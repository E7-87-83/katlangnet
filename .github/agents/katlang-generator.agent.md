...
- Output join: `left ; right`. In algorithm body/output context, newline-separated complete expressions also contribute output as if joined with semicolon.
- Sequence supply: `left...right` or postfix `expr...`; final `expr...` means `expr...empty` and does not continue onto the next line. `...` has lower precedence than `;`, so `Use(a ; b...)` means `Use((a ; b)...)`.
