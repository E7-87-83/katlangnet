...
- Ellipsis `...` is the sequence supply operator. `x...y` supplies the result sequence of `x` followed by the result sequence of `y`; bare sequence supply does not create a group, preserve or merge properties, or recursively flatten nested groups. Within the same line, spaces around expression-side `...` do not change meaning; a newline after postfix `...` completes `x...empty` instead of joining the next line. Postfix `x...` is shorthand for `x...empty`. Parentheses around sequence supply preserve one grouped result boundary: `Step = history...next` emits multiple results, while `Step = (history...next)` emits one grouped result. Explicit `empty` contributes no items; a no-output body such as `()` or `{}` is still an error. Use comma for ordinary multi-result data and argument slots, and use `...` only when sequence supplying evaluated result content is intended.
...
- For user-defined sequence-style helpers, declare an explicit variadic capture with postfix ellipsis, such as `Scale(values..., factor) = values.map{n * factor}`. The variadic capture consumes sibling values at its own comma-separated pattern level, may appear before suffix parameters, and preserves nested groups. Use explicit sequence supply when a receiver should provide several values to that variadic capture, such as `(Arg...).Scale(10)`. Use grouped parameter patterns when one fixed grouped slot should be opened, such as `Window((first, middle..., last), scale) = first * scale, middle.count, last * scale` or `NestedState(((history..., previous), current)) = history.count, previous, current`. A group pattern consumes one parent-level argument; its variadic capture does not spread across outer arguments. Do not use `atoms` unless recursive flattening is intentionally required.
...
- Sequence supply: same-line `left...right` or postfix `expr...`; a newline after postfix `...` ends the sequence supply as `expr...empty`.
...
- A normal parameter remains one ordinary argument boundary. With `Group(list) = list`, `Arg.Group.count` is `1` for `Arg = 1, 2, 3`; with `Group(list...) = list`, use `(Arg...).Group.count` to get `3`.
