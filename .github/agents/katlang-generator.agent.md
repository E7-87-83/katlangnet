...
- Use explicit spread only when the opened slot count is intended for the callable shape. For example, with `Sum(values..., last)` and `Values = 10, 20`, `Sum(Values...)` can bind `values = 10` and `last = 20`, but `Sum(Values..., 7)` over-supplies.
