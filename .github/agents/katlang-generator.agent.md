...
- Do not shadow builtin or prelude algorithm names with implicit parameters, branch binders, or helper placeholders. If a concept is naturally named `empty`, `atoms`, `ungroup`, `sum`, `min`, `max`, `avg`, `count`, `first`, `last`, `map`, `filter`, `order`, `orderDesc`, `reduce`, or `range`, rename it to a non-builtin alternative such as `noItems`, `flatValues`, `ungroupedValue`, `total`, `minimumValue`, `maximumValue`, `averageValue`, `itemCount`, `firstValue`, `lastValue`, `transform`, `predicate`, `sortedValues`, `descendingValues`, `reducer`, or `span`.
...
Do not use `atoms` to simulate `values...`; `atoms` recursively flattens nested structure and changes semantics. Use `atoms` only when recursive numeric atom extraction is actually requested. Use `ungroup(value)` or `value.ungroup` only when one fixed single value should expose exactly one outer content boundary; `ungroup(1, 2, 3)` is invalid, and nested groups remain grouped.
...
- `ungroup(value)` and `value.ungroup` perform that same one-level content projection on exactly one value. They are fixed-arity forms, not sequence builtins, so do not generate `ungroup(a, b, c)` expecting variadic capture
