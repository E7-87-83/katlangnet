...
- Semicolon `;` is the explicit output-stream joining operator. In root, algorithm, and brace output/body contexts, a physical newline between complete expressions also contributes output as if joined with semicolon, so report-style output can be written on separate lines. Same-line whitespace is never a separator: `1 2`, `Output = 1 2`, and `{ 1 2 }` are invalid. Comma has higher priority than semicolon: `1, 2 ; 3` emits `1, 2, 3`, not `1, (2 ; 3)`. Semicolon concatenates top-level output streams and does not create row/group boundaries; explicit parentheses are the grouping boundary, so `(1, 2) ; 3` emits grouped `(1, 2)` followed by `3`, and `(1, 2, 3)` remains one grouped value. Public result-window display may show multiple top-level outputs on separate visual rows for readability; that visual separation is presentation, not semantic grouping. Do not use newline joining inside call argument lists or explicit parenthesized groups.
- Ellipsis `...` is only the sequence supply/spread operator. `x...y` supplies the result sequence of `x` followed by the result sequence of `y`; postfix `x...` means `x...empty` and no longer continues onto the next line. Bare sequence supply does not create a group, preserve or merge properties, or recursively flatten nested groups. Spaces around expression-side `...` do not change meaning within one sequence-supply item. Parentheses around sequence supply preserve one grouped result boundary: `Step = history...next` emits multiple results, while `Step = (history...next)` emits one grouped result. `...` has lower precedence than `;`, so `Use(a ; b...)` means `Use((a ; b)...)`; write `Use(a ; (b...))` only when the supplied `b` stream should stay grouped as one joined operand. Explicit `empty` contributes no items; a no-output body such as `()` or `{}` is still an error. Use comma for ordinary multi-result data and argument slots, `;` for output joining, and `...` only when sequence supplying evaluated result content is intended.
...
- Output join: `left ; right`. In root, algorithm, and brace output/body contexts, physical-newline-separated complete expressions also contribute output as if joined with semicolon. Same-line whitespace is not a separator, and newline joining does not apply inside calls or explicit parenthesized groups. Semicolon concatenates top-level output streams; it does not preserve physical line boundaries as groups and does not flatten explicit grouped values. Result-window row display is presentation only and does not imply semantic grouping.
- Sequence supply: `left...right` or postfix `expr...`; final `expr...` means `expr...empty` and does not continue onto the next line. `...` has lower precedence than `;`, so `Use(a ; b...)` means `Use((a ; b)...)`; explicit grouping such as `Use(a ; (b...))` intentionally changes the shape.
