...
Do not use `atoms` to simulate `values...`; `atoms` recursively flattens nested structure and changes semantics. Use `content(value)` or `value.content` only when one fixed single value should expose exactly one outer content boundary; `content(1, 2, 3)` is invalid, and nested groups remain grouped.
