...
public class ParserTests
{
    ...
    [Theory]
    [InlineData("Author", "Mikus Vanags")]
    [InlineData("Version", "1.0")]
    [InlineData("Description", "KatLang library demonstration")]
    public void Parse_LibraryMetadataProperties_ArePrivateStringProperties(string name, string value)
    {
        var result = Parser.ParseSyntax($"{name} = '{value}'");
        ...
        var literal = Assert.IsType<Expr.StringLiteral>(Assert.Single(body.Output));
    ...
    public void Parse_PublicLibraryMetadataProperty_ReportsDiagnosticAndStaysPrivate(string name)
    {
        var result = Parser.ParseSyntax($"public {name} = 'demo'");

        Assert.True(result.HasErrors);
        Assert.Contains(result.Diagnostics, diagnostic =>
            diagnostic.Message == $"`{name}` is a reserved library metadata property and cannot be public.");
        var property = Assert.Single(result.Root.Properties);
        Assert.True(property.IsLibraryMetadata);
        Assert.False(property.IsPublic);
    }
    ...
    [Theory]
    [InlineData("Author = 123", "Author")]
    [InlineData("Version = 1.0", "Version")]
    [InlineData("Description = SomeValue", "Description")]
    public void Parse_LibraryMetadataProperty_NonStringLiteralReportsDiagnostic(string source, string name)
    {
        var result = Parser.ParseSyntax(source);

        Assert.True(result.HasErrors);
        Assert.Contains(result.Diagnostics, diagnostic =>
            diagnostic.Message == $"Library metadata property `{name}` must be a string literal.");
        var property = Assert.Single(result.Root.Properties);
        Assert.True(property.IsLibraryMetadata);
        Assert.False(property.IsPublic);
    }
    ...
    [Fact]
    public void Parse_NonMetadataPrivateProperty_IsUnaffected()
    {
        var result = Parser.ParseSyntax("Maintainer = 123");

        Assert.False(result.HasErrors, string.Join(Environment.NewLine, result.Diagnostics.Select(diagnostic => diagnostic.Message)));
        var property = Assert.Single(result.Root.Properties);
        Assert.Equal("Maintainer", property.Name);
        Assert.False(property.IsLibraryMetadata);
        Assert.False(property.IsPublic);
    }

    [Fact]
    public void Parse_NestedReservedMetadataName_IsOrdinaryProperty()
    {
        var result = Parser.ParseSyntax("Lib = { Author = 123 }");

        Assert.False(result.HasErrors, string.Join(Environment.NewLine, result.Diagnostics.Select(diagnostic => diagnostic.Message)));
        var lib = Assert.Single(result.Root.Properties);
        var body = Assert.IsType<Algorithm.User>(lib.Value);
        var author = Assert.Single(body.Properties);
        Assert.Equal("Author", author.Name);
        Assert.False(author.IsLibraryMetadata);
    }
