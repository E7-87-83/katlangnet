...
The plan-native flat variadic layout is still used for generic loop-step binding. User calls no longer use the strict one-slot variadic path: any top-level variadic now routes to the shared item-stream binder (below). User-call argument-expression evaluation, dot-call boundary preservation, and algorithm/value binding channels remain executor behavior.

Item-stream user calls — any callable with a top-level variadic capture, whether rest-only `G(x...)` or a comma deconstruction `F(x, y..., z)` — route to one executor path (`IsDeconstructionUserCallShape` / Lean `Algorithm.usesItemStreamBinding`). That path collects the call items, applies singleton-boundary normalization (`NormalizeSingletonBoundaryForItemStream` / `normalizeSingletonBoundaryForItemStream` — opens a stream that is exactly one grouped sequence value, opening that single boundary once without recursively renormalizing nested structure), then binds prefix/rest/suffix through the shared `BindParameterPatternList`. Rest-only is the degenerate single-rest case. Assignment deconstruction (`x, y..., z = RHS`) is parser-elaborated into properties that reuse the patterned binding path against a shared synthetic source property, so it shares the same `BindParameterPatternList` matcher rather than adding a third binder. Callback-element binding stays on the strict path (callback deconstruction deferred).
...
Builtin metadata uses `CallableSignature` and `CallableBindingPlan` where it is safe to describe builtin call shape. A rest-shaped sequence builtin (`sum(values...)`, `contains(values..., item)`) binds its top-level arguments through the **same** shared item-stream kernel as user variadics: `BindSequenceBuiltinArguments` / `bindSequenceBuiltinArguments` apply `NormalizeSingletonBoundaryForItemStream` and then `BindCallableArguments` (the fixed/suffix count is the rest-may-be-empty minimum), so `sum(1, 2, 3)`, `sum((1, 2, 3))`, and `count(Values...)` all consume the same stream. The collection is the rest capture, opened by that same single-boundary normalization (`NormalizeSingletonBoundaryForItemStream` / `normalizeSingletonBoundaryForItemStreamOf`): exactly one grouped sequence boundary is opened, without recursively renormalizing, so an explicitly nested empty like `(())` is preserved rather than collapsed, and sibling grouped values are preserved. What stays builtin-owned is the phase *after* binding: collection extraction shape constraints, suffix preparation, callback invocation, and result combination (`CombineCollectionResult` / `combineCollectionResult`, which keeps a single kept sequence-valued item as a one-item collection) and result-count rules.
