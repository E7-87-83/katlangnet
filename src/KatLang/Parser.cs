...
public sealed class Parser
{
    ...
    // Per-parse counter for synthetic shared-source properties generated by
    // deconstruction binding patterns (`x, y..., z = RHS`). Reset per parser
    // instance so the elaborated AST stays deterministic across re-parses.
    ...
    private Algorithm ParseAlgorithm(bool isParametrized)
    {
            ...
            && Current.Kind != TokenKind.RBrace)
        {
            ...
            // handled by the property branch above.
            else if (Current.Kind == TokenKind.Identifier && LookaheadIsBindingPatternAssignment())
            {
                ParseBindingPatternAssignment(properties, clauseGroups);
            }
    ...
    /// assignment: <c>Identifier ('...')? (',' Identifier ('...')?)+ '='</c>.
    /// Triggers only when the left-hand side has at least one comma (two or more
    /// targets). A plain single <c>name =</c> is left to the ordinary property path,
    /// and a single rest target <c>name... =</c> is not a binding pattern at all.
    /// </summary>
    private bool LookaheadIsBindingPatternAssignment()
    {
        ...
        while (true)
        {
            ...
            // Skip an optional rest marker on this target.
            ...
            switch (_tokens[index].Kind)
            {
                ...
                case TokenKind.Equals:
                    // Only a comma binding pattern (two or more targets) is a
                    // deconstruction assignment. A single `name... = RHS` is not a
                    // valid assignment form and falls through to the output path.
                    return sawComma;
                default:
                    return false;
            }
    ...
    /// <summary>
    /// Parses a comma deconstruction assignment <c>x, y..., z = RHS</c>. The
    ...
    {
        var targets = new List<(string Name, ParameterKind Kind, SourceSpan Span)>();
        while (true)
        {
            ...
            targets.Add((name, kind, TokenSpan(nameToken)));
        ...
        foreach (var target in targets)
        {
            ...
            {
                ReportError($"Property '{target.Name}' is already defined.", target.Span);
            }
        }

        AddDeconstructionProperties(targets, rhsBody, properties);
    }
    ...
    /// the ordinary patterned-binding path, so a single grouped right-hand side is
    /// opened and matched element-by-element. Synthetic constructs carry no source
    /// spans; only the target property names are source-backed declarations.
    /// </summary>
    private void AddDeconstructionProperties(
        IReadOnlyList<(string Name, ParameterKind Kind, SourceSpan Span)> targets,
        Algorithm rhsBody,
        List<Property> properties)
    {
